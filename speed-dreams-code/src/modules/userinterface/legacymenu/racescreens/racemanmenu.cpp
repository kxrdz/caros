...
// Menu control Ids
static int RaceModeTitleLabelId;
...
rmOnRaceDataChanged()
{
	...
	const char *fmt = tr("modecircuit");
	char buf[256];
	std::string key = "racemode/" + pRaceMan->getName();
	const char *text = retr(key.c_str());

	if (!*text)
		text = pRaceMan->getName().c_str();

	snprintf(buf, sizeof(buf), fmt, text,
		pTrack->getName().c_str());

	// Set title (race type + track name).
	GfuiLabelSetText(ScrHandle, RaceModeTitleLabelId, buf);
...
rmOnPlayerConfig(void * /* dummy */)
{
	...
	if (pRaceMan->acceptsDriverType(ROB_VAL_HUMAN))
		new HumanSelect(ScrHandle, pRace, true);
	else
		RmDriversSelect(ScrHandle);
...
RmRacemanMenu()
{
	...
	ScrHandle = GfuiScreenCreate(NULL, NULL, rmOnActivate,
										 NULL, (tfuiCallback)NULL, 1);
	menuXMLDescHdle = GfuiMenuLoad("racemanmenu.xml");
