...
public:
	...
	virtual int onRaceDriversLoaded();
	...
	virtual void setResultsTableHeader(const std::vector<std::string> &header);
	virtual void addResultsTableRow(const std::vector<std::string> &row);
	virtual void setResultsTableRow(int nIndex, const std::vector<std::string> &row, bool bHighlight = false);
	...
	// Add human car to pre-race configuration.
	virtual void addHumanCfg(const std::vector<tCarElt *> &cars, void *hook);
...
public:
 ...
 protected:
	...
	//! Load and activate the main menu.
	static bool runMainMenu();

	//! Activate the welcome menu.
	static bool activateWelcomeMenu();
