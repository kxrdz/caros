...
bool LegacyMenu::runMainMenu()
{
    return backLoad() && MainMenuRun() == 0;
}

bool LegacyMenu::activateWelcomeMenu()
{
    new WelcomeMenu(runMainMenu);
    return true;
}

const char *LegacyMenu::tr(const char *key) const
{
    void *param = LmRaceEngine().inData()->raceEngineInfo.param;
...
bool LegacyMenu::activate()
{
    ...
    bool runWelcome = WelcomeMenu::shouldRun();

    if (runWelcome)
        fnSplashBackWork = nullptr;
	...
	{
        if (runWelcome)
            fnOnSplashClosed = LegacyMenu::activateWelcomeMenu;
        else
            // If not specified, simply open the splash screen, load the menus
            // in the background and finally open the main menu.
...
void LegacyMenu::activateLoadingScreen()
{
    ...
    std::string key = std::string("racemode/") + pReInfo->_reName;

	::RmLoadingScreenStart(tr(key.c_str()), "data/img/splash-raceload.jpg");
}
...
void LegacyMenu::addHumanCfg(const std::vector<tCarElt *> &cars, void *hook)
{
    if (_piRaceEngine->inData()->_displayMode == RM_DISP_MODE_NORMAL)
        RmAddHumanCfg(cars, hook);
}
...
int LegacyMenu::onRaceDriversLoaded()
{
    if (_piRaceEngine->inData()->_displayMode == RM_DISP_MODE_NORMAL)
	{
		...
		{
            char buf[128];
            const char *fmt = tr("loading/graphicstrack");
            snprintf(buf, sizeof (buf), fmt,
                    _piRaceEngine->inData()->track->name);
            addLoadingMessage(buf);

            // Initialize the track graphics.
            if (!loadTrackGraphics(_piRaceEngine->inData()->track))
                return -1;
        }
    }
...
void LegacyMenu::onRaceSimulationReady()
{
    if (_piRaceEngine->inData()->_displayMode == RM_DISP_MODE_NORMAL)
	{
        ...
        addLoadingMessage(tr("loading/graphicscars"));

        loadCarsGraphics(_piRaceEngine->outData()->s);
        addLoadingMessage(tr("loading/soundcars"));
...
void LegacyMenu::setResultsTableHeader(const std::vector<std::string> &header)
{
    ::RmResScreenSetHeader(header);
}

void LegacyMenu::addResultsTableRow(const std::vector<std::string> &row)
{
    ::RmResScreenAddText(row);
}

void LegacyMenu::setResultsTableRow(int nIndex, const std::vector<std::string> &row, bool bHighlight)
{
    ::RmResScreenSetText(row, nIndex, bHighlight ? 1 : 0);
}
