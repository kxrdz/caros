...
cGrBoard::cGrBoard(int myid) :
    ...
    param(GfuiMenuLoad("grboard.xml"))
{
    if (!param)
        throw std::runtime_error("Could not open grboardmenu.xml");
...
cGrBoard::loadDefaults(const tCarElt *curCar)
{
    ...
    deltaFlag   = (int)GfParmGetNum(grHandle, path, GR_ATT_DELTA, NULL, 1);
...
cGrBoard::grDispDebug(const tSituation *s, const cGrFrameInfo* frame)
{
    ...
    if (debugFlag == 2) {
        ...
        snprintf(buf, sizeof(buf),  "%s: %.f", tr("time"), s->currentTime);
        GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, x2, y);
    } else if (debugFlag == 3) {
        // Only display detailed information in Debug Mode > 1
        // Display segment name in 2nd column
        snprintf(buf, sizeof(buf),  "%s: %s", tr("seg"), car_->_trkPos.seg->name);
        GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, x2, y);

        // Display distance from start in 2nd row, 1st column
        y -= dy;
        snprintf(buf, sizeof(buf), "%s: %5.0f", tr("dfs"), car_->_distFromStartLine);
        GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, x, y);

        // Display current camera in 2nd row, 2nd column
        tRoadCam *curCam = car_->_trkPos.seg->cam;
        if (curCam) {
            snprintf(buf, sizeof(buf), "%s: %s", tr("cam"), curCam->name);
            GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, x2, y);
        }
    } else if (debugFlag == 4) {
        int x;

        strcpy(buf, ssgShowStats());
        /* ssgShowStats() appends a final '\n' that must not be rendered */
        *strchr(buf, '\n') = '\0';
        x = rightAnchor - GfuiFontWidth(GFUI_FONT_SMALL_C, buf);
        GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, x, y - dy);
    }
}  // grDispDebug
...
cGrBoard::grDispGGraph()
{
    ...
    {
        char buf[BUFSIZE];
        snprintf(buf, sizeof(buf), "P: %3.2f", car_->_tyreCurrentPressure(1) / 6894.76f);
        ...
        snprintf(buf, sizeof(buf), "P: %3.2f", car_->_tyreCurrentPressure(0) / 6894.76f);
        ...
        GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, X1 - 50.00, Y1 - 15.0);
        snprintf(buf, sizeof(buf), "P: %3.2f", car_->_tyreCurrentPressure(3) / 6894.76f);
        GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, X1 - 50.00, Y1 - 25.0);
        snprintf(buf, sizeof(buf), "T: %3.2f", K2C(car_->_tyreT_mid(2)));
        GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, X1 + 10.00, Y1 - 15.0);
        snprintf(buf, sizeof(buf), "P: %3.2f", car_->_tyreCurrentPressure(3) / 6894.76f);
        GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, X1 + 10.00, Y1 - 25.0);
        snprintf(buf, sizeof(buf), "%s: %3.2f", tr("airtemp"), K2C(car_->_airtemp));
...
cGrBoard::grDispDeltaBestLap(const tSituation *s)
{
    ...
    static const int barBottomY = 560; // this should depend on the (split) window height
    ...
    {
        const char *text = tr("waitingvalidlap");
        int font = GFUI_FONT_BIG, width = GfuiFontWidth(font, text),
            x = barCenterX - width / 2;

        GfuiDrawString(text, normal_color_, font, x, barBottomY);
    }
}
...
cGrBoard::grDispCarBoard1(const tSituation *s)
{
    ...
    // Display fuel
    snprintf(buf, sizeof(buf), "%s:", tr("fuel"));
    GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, x, y);
    ...
    snprintf(buf, sizeof(buf), "%s:", tr("damage"));
...
cGrBoard::grDispCarBoard2(const tSituation *s)
{
    ...
    snprintf(buf, sizeof(buf), "%s:", tr("time"));
    GfuiDrawString(buf, normal_color_, GFUI_FONT_SMALL_C, x, y);
...
// grDispCarBoard3
//
// Displays driver-specific info (version 3)
//
// @param s[in] A pointer to the current situation
// @param car[in] A pointer to the current driver's car
// @return void
...
cGrBoard::grDispCarBoard3(const tSituation *s)
{
    ...
    if (car_->_pos > 1)
    {
        ...
        const tCarElt *f = s->cars[car_->_pos - 2];
        int fdist = f->_distFromStartLine, cdist = car_->_distFromStartLine;
        float *t = f->_currLapTimeAtTrackPosition,
            ftime = t[fdist],
            ctime = t[cdist];

        // if he is in my same lap
        if (f->_laps == car_->_laps)
        {
            // get how much time it took him to reach his position from
            // my position
            deltaAhead = ftime - ctime;
            deltaAhead = - deltaAhead;
        }
        else
        {
            // if he is on the next lap or many laps ahead
            int lapsAhead = f->_laps - car_->_laps;

            // if he has not lapped me
            if (car_->_distFromStartLine > f->_distFromStartLine)
            {
                // for simplicity we consider his last laptime
                // (this may be totally wrong if he for example pitted
                // in the last lap)
                deltaAhead = (lapsAhead -1) * f->_lastLapTime;
                // how much time took him to reach the startline from
                // my position
                deltaAhead += f->_lastLapTime - ctime;
                // how much time took him to reach his position from
                // the startline
                deltaAhead += ftime;
                deltaAhead = - deltaAhead;
            // he has lapped me
            ...
            {
                // for simplicity we consider his last laptime (this may be
                // totally wrong if he for example pitted in the last lap)
                deltaAhead += lapsAhead * f->_lastLapTime;
                // much time took him to reach his position from my position
                // in this lap
                deltaAhead += ftime - ctime;
                deltaAhead = - deltaAhead;
            }
    ...
    if (car_->_pos < s->_ncars)
    {
        ...
        // first car behind
        const tCarElt *f = s->cars[car_->_pos];

        int cdist = car_->_distFromStartLine, fdist = f->_distFromStartLine;
        float *t = car_->_currLapTimeAtTrackPosition,
            ctime = t[cdist], ftime = t[fdist];

        // if he is in my same lap
        if (f->_laps == car_->_laps)
        {
            // get how much time it took me to reach my position from
            // my position
            deltaBehind = ctime - ftime;
        }
        else
        {
            // if he is on the prev lap or many laps Behind
            int lapsBehind = car_->_laps - f->_laps;
            ...
            if (car_->_distFromStartLine < f->_distFromStartLine)
            {
                ...
                // how much time took us to reach the startline from
                // his position
                deltaBehind += car_->_lastLapTime - ftime;
                // how much time took us to reach our position from
                // the startline
                deltaBehind += ctime;
            // we have lapped him
            }
            else
            {
                ...
                // how much time took us to reach our position from
                // his position in this lap
                deltaBehind += ftime - ctime;
            }
        }
    }
    ...
    if (car_->_pos != 1)
    {
        const tCarElt *c = s->cars[car_->_pos - 2];

        snprintf(buf, sizeof(buf), "%s", c->_sname);
        GfuiDrawString(buf, color, GFUI_FONT_SMALL_C, x, y);
        ...
        else
        {
            ...
            {
                grWriteTime(color, GFUI_FONT_SMALL_C, x3, y, dxc,
                            car_->_bestLapTime - c->_bestLapTime, 1);
            }
            else
            {
                GfuiDrawString("--:---", color, GFUI_FONT_SMALL_C, x3, y, dxc,
    ...
    if (car_->_pos != s->_ncars)
    {
        const tCarElt *c = s->cars[car_->_pos];
        ...
        {
            if (c->_bestLapTime > 0.0f)
            {
                grWriteTime(color, GFUI_FONT_SMALL_C, x3, y, dxc,
                           c->_bestLapTime - car_->_bestLapTime, 1);
            }
            else
            {
                GfuiDrawString("--:---", color, GFUI_FONT_SMALL_C, x3, y, dxc,
        ...
        if (drawLaps) {
            if (s->_raceType == RM_TYPE_RACE) {
                if (s->_totTime > s->currentTime) {
                    snprintf(buf, sizeof(buf), " %s:", tr("laps"));
                    GfuiDrawString(buf, emphasized_color_, GFUI_FONT_SMALL_C, x, y);
                    snprintf(buf, sizeof(buf), "%d", MAX(s->cars[0]->_laps-1, 0));
                } else {
                    snprintf(buf, sizeof(buf), " %s:", tr("lap"));
                    GfuiDrawString(buf, emphasized_color_, GFUI_FONT_SMALL_C, x, y);  //yellow
                ...
                if (s->_totTime > 0.0f) {
                    ...
                    snprintf(buf, sizeof(buf), "%s:", tr("timeleft"));
                    GfuiDrawString(buf, emphasized_color_, GFUI_FONT_SMALL_C, x, y);
                                     ...
                                     const bool isLeader) const
{
    ...
    if (car->_state & RM_CAR_STATE_DNF)  {
        snprintf(buf, sizeof(buf), "       %s", tr("out"));
        return buf;
    }

    if (car->_state & RM_CAR_STATE_PIT) {
        snprintf(buf, sizeof(buf), "       %s", tr("pit"));
        return buf;
    }
    ...
    case 1:  // 1 lap behind leader
        snprintf(buf, sizeof(buf), "+%3d %s", lapsBehindLeader, tr("lap"));
        break;

    default:  // N laps behind leader
        snprintf(buf, sizeof(buf), "+%3d %s", lapsBehindLeader, tr("laps"));
        break;
    }
...
cGrBoard::grDispDashboard()
{
    ...
    else
    {
        ...
        case DI_TYRE_SET:
            ...
            {
                snprintf(buf2, sizeof(buf2), "%s", tr("yes"));
            } else {
                snprintf(buf2, sizeof(buf2), "%s", tr("no"));
            }
            ...
            //if (item->setup->desired_value > 0)
        {
            ...
            {
                snprintf(buf2, sizeof(buf2), "%s", tr("compounds/soft"));
            }
            else if (item->setup->desired_value == 2)
            {
                snprintf(buf2, sizeof(buf2), "%s", tr("compounds/medium"));
            }
            else if (item->setup->desired_value == 3)
            {
                snprintf(buf2, sizeof(buf2), "%s", tr("compounds/hard"));
            }
            else if (item->setup->desired_value == 4)
            {
                snprintf(buf2, sizeof(buf2), "%s", tr("compounds/wet"));
            }
            else if (item->setup->desired_value == 5)
            {
                snprintf(buf2, sizeof(buf2), "%s", tr("compounds/extwet"));
            }
        }
            snprintf(buf3, sizeof(buf3), "%s", "");
            break;
        ...
        case DI_PENALTY:
            buf1 = tr("boxtype");
            if (item->setup->desired_value > 0.9) {
                snprintf(buf2, sizeof(buf2), "%s", tr("boxtype/penalty"));
            } else {
                snprintf(buf2, sizeof(buf2), "%s", tr("boxtype/repair"));
            }
            snprintf(buf3, sizeof(buf3), "%s", "");
            break;
        }
...
const char *cGrBoard::tr(const char *key) const
{
