...
static std::string strBrakeRep("brakerep");
...
static std::string strPenalty("boxtype");
...
void SDHUD::CreateHUD(int scrH, int scrW)
{
    ...
    paramHandle = GfParmReadFileLocal("config/osghudconfig.xml", GFPARM_RMODE_REREAD);
    ...
    // reset car data from previous races
    carData.clear();
    prevSteerAngle = 0.0f;
}
...
std::string &SDHUD::GetCarName(const tCarElt *currCar)
{
    CarData &d = carData[currCar];
    std::string &s = d.name;

    if (s.empty())
    {
        std::string p = "cars/models/";
        ...
        void *h = GfParmReadFileBoth(path, GFPARM_RMODE_STD);
...
const char *SDHUD::tr(const char *key)
{
    if (!paramHandle)
        paramHandle = GfParmReadFileLocal("config/osghudconfig.xml", GFPARM_RMODE_STD);

    std::string s = std::string("translatable/") + key;

    return GfuiGetTranslatedText(paramHandle, s.c_str(), "text", "");
}
                        ...
                        const tCarElt *currCar, int SimClouds, int SimRain, float SimTimeOfDay)
{
    ...
    const auto encoding = osgText::String::Encoding::ENCODING_UTF8;
...
//board
    ...
    for(std::vector<tCarElt *>::iterator car = boardCars.begin(); car != boardCars.end(); ++car) {
        ...
        mapKey.str("");
        mapKey << "board-player" << id << "-name";
        hudTextElements[mapKey.str()]->setText((*car)->_name, encoding);
        hudTextElements[mapKey.str()]->setNodeMask(NODE_MASK_ALL);
...
//gear
    ...
    const char *unit= GfParmGetStr(paramHandle, "settings/speed", "unit", "kph");
    double absspeed = fabs(currCar->_speed_x);
    int speed = !strcmp(unit, "kph") ? absspeed * 3.6 : absspeed * 2.237;
//speed
    temp.str("");
    //get speed in km/h
    temp << speed;
    ...
    //set speed unit
    hudTextElements["speed-number-unit"]->setText(unit);
...
// dash items
    ...
    if (currCar->_dashboardActiveItem < currCar->_dashboardInstantNb)
    {
        ...
        hudTextElements["dash-items-type"]->setText(tr(description.c_str()), encoding);
    ...
    else
    {
        ...
        case DI_PENALTY:
            ...
            if (item->setup->desired_value > 0.9)
            {
                value1 << tr("boxtype/penalty");
            }
            else
            {
                value1 << tr("boxtype/repair");
            }
            break;
        }
        hudTextElements["dash-items-type"]->setText(tr(description.c_str()), encoding);
        hudTextElements["dash-items-value1"]->setColor(colorYellow);
        hudTextElements["dash-items-value1"]->setText(value1.str(), encoding);
    ...
    if (hudWidgets["shiftlightsWidget"]->getNodeMask()){
        ...
        */
        ...
        if(currCar->_enginerpm > 0 && currCar->_gear > 0 ){
            ...
            float horsepowerCurr = 0;
            float horsepowerNext = 0;

            if (horsepowerPoints.size() > (unsigned)currCar->_enginerpm)
                horsepowerCurr = horsepowerPoints.at((int)currCar->_enginerpm);

            if (horsepowerPoints.size() > (unsigned)nextRpm)
                horsepowerNext = horsepowerPoints.at((int)nextRpm);
...
//driverinfo
        ...
        hudTextElements["driverinfo-carmodel"]->setText(GetCarName(currCar));
            ...
            if (GfParmListSeekFirst(paramHandle, widgetsSectionPath.c_str()) == 0)
            {
                do
                {
                        ...
                        /* ============================
                             CREATE OSG TEXT
                           ============================*/
                        if (type == GFMNU_ATTR_TEXT)
                        {
                            //read data into local variables
                            const std::string &elementId = subSectionName;
                            std::string textStr =          GfuiGetTranslatedText (paramHandle, subSectionPath.c_str(), GFMNU_ATTR_TEXT, "" );
                            std::string fontFileUrl =      GfuiGetTranslatedText(paramHandle, subSectionPath.c_str(),"fontFileUrl", "" );
                            ...
                            //set the text string
                            text->setText(textStr, osgText::String::Encoding::ENCODING_UTF8);
