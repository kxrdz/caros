...
bool StandardGame::loadPhysicsEngine()
{
	...
	// 1) Get the physics engine name from user settings (default: Simu V5.0)
	...
	// 3) Load it.
	const char *fmt = ReTr("loading/physicsengine");
	char buf[128];

	snprintf(buf, sizeof(buf), fmt, strModName.c_str());

	if (_piUserItf)
		_piUserItf->addLoadingMessage(buf);
