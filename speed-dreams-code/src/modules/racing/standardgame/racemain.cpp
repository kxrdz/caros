...
ReTr(const char *key)
{
    void *param = ReInfo->raceEngineInfo.param;
...
int
ReFindDriverIdx (const char *modulename, const char *drivername)
{
    char path[128];
    void *params = ReInfo->params;
    ...
    for (int i = 1; i <= GfParmGetEltNb(params, RM_SECT_DRIVERS); i++) {
        ...
        const char *parm_name = GfParmGetStr(params, path, RE_ATTR_DRIVERNAME, ""),
            *parm_mod = GfParmGetStr(params, path, RE_ATTR_MODULE, "");

        if ((strcmp(drivername, parm_name) == 0) &&
             (strcmp(modulename, parm_mod) == 0) ) {
            //car found
            return i;
        }
    }
...
RePreRace(void)
{
    ...
    {
        ReUI().addLoadingMessage(ReTr("loading/startingorder"));
            ...
            for (int i = 1; i < nCars + 1; i++)
            {
                ...
                ReStartingOrderIdx[i-1] =
                    ReFindDriverIdx (GfParmGetStr(params, path, RE_ATTR_MODULE, ""),
                                    GfParmGetStr(params, path, RE_ATTR_DRIVERNAME, ""));
            }
        }
    }
...
RePreRacePause(void)
{
    ...
    if (RM_DISP_MODE_NORMAL == ReInfo->_displayMode)
    {
        ...
        {
            const char *text = ReSituation::self().tr("hitenter");
            mode = RM_ASYNC | RM_NEXT_STEP;
            //ReStop();
            ReSituation::self().setRaceMessage(text,-1/*always*/, /*big=*/true);
            ReStop();
        }
    }
    return mode;
...
ReRaceRealStart(void)
{
    ...
    for (i = 0; i < s->_ncars; i++)
    {
        const char *fmt = ReTr("loading/driver");
    ...
    ReUI().addLoadingMessage(ReTr("loading/prestart"));
    ...
    if (NetGetNetwork())
    {
        ReUI().addLoadingMessage(ReTr("loading/onlinerace"));
    ...
    ReUI().addLoadingMessage(ReTr("loading/ready"));
...
ReRaceStart(void)
{
    ...
    //Checks if there is only one driver per session allowed, so practice, qualification without timed session.
    {
        ...
        GfParmSetStr(params, path2, RM_ATTR_DRIVERNAME,
                     GfParmGetStr(params, path, RM_ATTR_DRIVERNAME, ""));
    ...
    else
    {
        ...
        ReUI().addLoadingMessage(ReTr("loading/startinggrid"));
...
ReRaceCooldown()
{
    ...
    if (RM_DISP_MODE_NORMAL == ReInfo->_displayMode)
    {
        ...
        {
            const char *text = ReSituation::self().tr("showresults");

            ReSituation::self().setRaceMessage(text, -1, /*big=*/true);
            // enable cooldown
            mode = RM_ASYNC;
        }
    }
