...
ReCarsUpdateCarPitTime(tCarElt *car)
{
    ...
    switch (car->_pitStopType)
    {
        ...
        case RM_PIT_STOPANDGO:
            penalty = GF_TAILQ_LAST(&(car->_penaltyList), CarPenaltyHead);
...
reCarsAddPenalty(tCarElt *car, int penalty)
{
    ...
    if (penalty == RM_PENALTY_SLOWDOWN)
        tr = ReSituation::self().tr("penalty/slowdown/start");
    else if (penalty == RM_PENALTY_DRIVETHROUGH)
        tr = ReSituation::self().tr("penalty/drive-through");
    else if (penalty == RM_PENALTY_STOPANDGO)
        tr = ReSituation::self().tr("penalty/stop-and-go");
    else if (penalty == RM_PENALTY_10SEC_STOPANDGO)
        tr = ReSituation::self().tr("penalty/stop-and-go-10sec");
    else if (penalty == RM_PENALTY_DISQUALIFIED)
        tr = ReSituation::self().tr("penalty/disqualified");

    snprintf(msg, sizeof(msg), "%s: %s", car->_name, tr);
    ...
    // Initially, the newPenalty->timeToClear field is 0.
    // This field is the relative diff, and it will be
    // added to the current cronometer when suitable
    if (penalty == RM_PENALTY_SLOWDOWN) {
        newPenalty->lapToClear = car->_laps + 2;
    } else {
...
static double penaltyFactor(tCarPenalty *penalty) {
    if (penalty == NULL)
        return 1.0;
    if (penalty->thr2)
        return RM_SLOW_THRESHOLD_2_FACTOR;
    if (penalty->thr1)
        return RM_SLOW_THRESHOLD_1_FACTOR;
    return 1.0;
}

static bool reCarsPenaltyEnabled(const tCarElt *car, int penalty)
{
    return !(car->_disabledPenalties & (1 << penalty));
}

/* Compute the race rules and penalties */
static void
reCarsApplyRaceRules(tCarElt *car, double deltaTime)
{
    char msg[64];
    tCarPenalty		*penalty;
    ...
    static const float	ctrlMsgColor[] = {1.0, 1.0, 1.0, 1.0};
    ...
    if ((car->pub.state & RM_CAR_STATE_FINISH) == 0)
    {
        ...
        if (car->_commitBestLapTime
            && car->_simcollision & SEM_COLLISION_XYSCENE
            && ((ReInfo->raceRules.enabled & RmRaceRules::WALL_HIT_TIME_INVALIDATE)
                && reCarsPenaltyEnabled(car, RM_PENALTY_WALLHIT_TIME_INVALIDATE)))
        {
            const char *tr = ReSituation::self().tr("penalty/hitwall");
    ...
    penalty = GF_TAILQ_LAST(&(car->_penaltyList), CarPenaltyHead);
    bool lastPenaltyIsSlowdown = penalty && penalty->penalty == RM_PENALTY_SLOWDOWN;
    tdble cuttinglimit = car->_dimension_y*0.7f;
    if (toborder < -cuttinglimit)
    {
        // Do not add another penalty if the current one hasn't been served yet.
        // Instead, accrue penalty time.
        if (lastPenaltyIsSlowdown) {
            // If the penalty was clearing and we went offtrack again, reset
            // the threshold flags
            if (penalty->clearing == true) {
                penalty->thr1 = false;
                penalty->thr2 = false;
                penalty->accum_thr1 = 0.0;
                penalty->accum_thr2 = 0.0;
            }
            penalty->clearing = false;

            double cutDistance = (-toborder) - cuttinglimit; // How much did we cut by? In meters
            // if it's <=1meter, assume 1 meter.
            if (cutDistance < 1) cutDistance = 1;

            // 100 meters offtrack at cutDistance=1 gives 5 seconds of penalty
            double constant = 5.0/100.0;
            double speed = sqrt(car->_speed_x*car->_speed_x + car->_speed_y*car->_speed_y + car->_speed_z*car->_speed_z);

            if (speed < RM_SLOW_THRESHOLD_1)
                penalty->accum_thr1 += deltaTime;
            if (speed < RM_SLOW_THRESHOLD_2)
                penalty->accum_thr2 += deltaTime;

            // The instant a certain threshold's minmum time is reached,
            //  multiply the penalty by its factor.
            // If threshold 2 is already active, threshold 1 is ignored.
            if (!penalty->thr2 && penalty->accum_thr2 > RM_SLOW_THRESHOLD_2_TIME) {
                const char *tr = ReSituation::self().tr("penalty/slowdown/forgiven");

                penalty->thr2 = true;
                penalty->timeToClear = penalty->timeToClear*penaltyFactor(penalty);

                snprintf(msg, sizeof(msg), "%s: %s", car->_name, tr);
                ReSituation::self().setRaceMessage(msg, 5);
            }
            if (!penalty->thr2 && !penalty->thr1 && penalty->accum_thr1 > RM_SLOW_THRESHOLD_1_TIME) {
                penalty->thr1 = true;
                penalty->timeToClear = penalty->timeToClear*penaltyFactor(penalty);
            }

            penalty->timeToClear += log2(cutDistance+1) * constant * speed * deltaTime * penaltyFactor(penalty);
            double maxPenalty = 15*penaltyFactor(penalty);
            if (penalty->timeToClear > maxPenalty) penalty->timeToClear = maxPenalty;

            if (penalty->timeToClear > 0.1 && ((int)(penalty->timeToClear*10) % 5) == 0) {
                const char *tr = ReSituation::self().tr("penalty/slowdown/start");

                //Only refresh penalty message if the accrued time has a decimal digit of .5 or .0
                snprintf(msg, sizeof(msg), "%s: %s (%0.1fs)", car->_name, tr, penalty->timeToClear);
                ReSituation::self().setRaceMessage(msg, 5);
            }
        } else if (reCarsPenaltyEnabled(car, RM_PENALTY_SLOWDOWN)) {
            car->_commitBestLapTime = false;
            reCarsAddPenalty(car, RM_PENALTY_SLOWDOWN);
            GfLogInfo("%s Got a slowdown.\n", car->_name);
        }
    } else if (lastPenaltyIsSlowdown){
        if (penalty->clearing == false) {
            if (penalty->timeToClear > 0.1 && penalty->timeToClear < 3)  penalty->timeToClear = 3;
            const char *tr = ReSituation::self().tr("penalty/slowdown/clearing");

            snprintf(msg, sizeof(msg), "%s: %s (%0.1fs)",
                                        car->_name, tr, penalty->timeToClear);
            ReSituation::self().setRaceMessage(msg, penalty->timeToClear);
            penalty->clearing = true;
        } else if (penalty->clearing == true) {
            if (penalty->timeToClear <= 0) {
                const char *tr = ReSituation::self().tr("penalty/slowdown/cleared");

                penalty->timeToClear = 0;
                snprintf(msg, sizeof(msg), "%s: %s", car->_name, tr);
                ReSituation::self().setRaceMessage(msg, 5);
                reCarsRemovePenalty(car, penalty);
                GfLogInfo("%s should get its slowdown penalty cleared.\n", car->_name);
            } else {
                if (car->ctrl.accelCmd > 0.2) car->ctrl.accelCmd = 0.2;
                penalty->timeToClear -= deltaTime;
            }
        }
    }
    ...
    if (penalty)
    {
        ...
        {
            case RM_PENALTY_SLOWDOWN:
                snprintf(car->ctrl.msg[3], RM_CMD_MAX_MSG_SIZE, "Slowdown Penalty");
                break;
    ...
    if (penalty)
    {
        if (reCarsPenaltyEnabled(car, RM_PENALTY_DISQUALIFIED)
            && car->_laps > penalty->lapToClear)
        {
        ...
        if (penalty)
        {
            ...
            if (seg->raceInfo & TR_PIT)
            {
                switch (penalty->penalty)
                {
                    case RM_PENALTY_DRIVETHROUGH:
                    {
                        const char *tr = ReSituation::self().tr("penalty/drive-through/clearing");
                    ...
                    case RM_PENALTY_10SEC_STOPANDGO:
                    {
                        const char *tr = ReSituation::self().tr("penalty/stop-and-go/clearing");
    ...
    else if (prevSeg->raceInfo & TR_PIT)
    {
        ...
        if (seg->raceInfo & TR_PIT)
        {
            ...
            {
                const char *tr = ReSituation::self().tr("penalty/cleared");
            ...
            if (reCarsPenaltyEnabled(car, RM_PENALTY_STOPANDGO)
                && !(rules->ruleState & RM_PNST_STOPANDGO))
            {
    ...
    {
        if (reCarsPenaltyEnabled(car, RM_PENALTY_DRIVETHROUGH)
            && !(rules->ruleState & (RM_PNST_OVERSPEED | RM_PNST_STOPANDGO))
...
static void
ReCarsSendLap(const tCarElt *car)
{
    ...
    bool valid = car->_commitBestLapTime;
    int water = ReSituation::self().data()->track->local.water;

    w.sendLap(w.raceId, valid, car->_lastLapTime, car->_fuel, car->_pos, water);

    if (car->_remainingLaps < 0)
        w.sendRaceEnd(w.raceId, car->_pos);
}

void
ReCarsManageCar(tCarElt *car, bool& bestLapChanged, double deltaTime)
{
    char msg[64];
    int i;
    int xx;
    ...
    if (car->_pit) {
        ...
        if (car->_state & RM_CAR_STATE_PIT) {
            ...
            if (car->_scheduledEventTime > 0.0) {
                if (car->_scheduledEventTime < s->currentTime) {
                    const char *tr = ReSituation::self().tr("pitstop");

                    car->_state &= ~RM_CAR_STATE_PIT;
                    car->_pit->pitCarIndex = TR_PIT_STATE_FREE;
                    snprintf(msg, sizeof(msg), "%s %s %.1f s", car->_name, tr, info->totalPitTime);
                    msg[sizeof(msg)-1] = 0; // Some snprintf implementations fail to do so.
                    ReSituation::self().setRaceMessage(msg, 5);
        ...
        // (position, speed, ...) and start up pitting process if so.
            ...
            if ((lgFromStart > car->_pit->lmin) && (lgFromStart < car->_pit->lmax)) {
                ...
                {
                    const char *tr = ReSituation::self().tr("inpits");
            ...
            {	// The car's position along the track is out of accepted range
                ...
                {
                  const char *text = ReSituation::self().tr("stilltogo");
                  float StillToGo = car->_pit->lmin - lgFromStart;
                  snprintf(car->ctrl.msg[2], RM_CMD_MAX_MSG_SIZE, "%s: %0.2f m" , text, StillToGo);
                  car->ctrl.msg[2][RM_CMD_MAX_MSG_SIZE-1] = 0; // Some snprintf implementations fail to do so.
                  if (TeamDriver)
                ...
                {
                    const char *text = ReSituation::self().tr("overrun");
                ...
                if (!(car->_state & RM_CAR_STATE_FINISH))
                {
                    ...
                    if ((car->_remainingLaps < 0 && s->currentTime > s->_totTime) || (s->_raceState == RM_RACE_FINISHING))
                    {
                        ...
                        if (ReInfo->s->_raceType == RM_TYPE_RACE)
                        {
                            if (car->_pos == 1)
                            {
                                const char *tr = ReSituation::self().tr("winner");
                                ...
                                const char *numSuffix,
                                    *trsuff = "default",
                                    *tr = ReSituation::self().tr("finished");
                                std::string section;
                                ...
                                section = std::string("possuffix/") + trsuff;
                                numSuffix = ReSituation::self().tr(section.c_str());
                                snprintf(msg, sizeof(msg), "%s %s %d%s", car->_name, tr, car->_pos, numSuffix);
                                msg[sizeof(msg)-1] = 0; // Some snprintf implementations fail to do so.
                                ReSituation::self().setRaceMessage(msg, 5);
                            }
                        }
    ...
    reCarsApplyRaceRules(car, deltaTime);
...
ReCarsSortCars(void)
{
    ...
    if (s->cars[i]->_wrongWayTime < s->currentTime
        ...
        const char *tr = ReSituation::self().tr("wrongway");

        snprintf(msg, sizeof(msg), "%s: %s", s->cars[i]->_name, tr);
        msg[sizeof(msg)-1] = 0; // Some snprintf implementations fail to do so.
