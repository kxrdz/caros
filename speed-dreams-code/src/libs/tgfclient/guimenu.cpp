...
createStaticImage(void* hscr, void* hparm, const char* pszName,
                  bool bFromTemplate = false, int x = GFUI_TPL_X,
                  int y = GFUI_TPL_Y, int w = GFUI_TPL_WIDTH,
                  int h = GFUI_TPL_HEIGHT)
{
    const char* pszImage = GfParmGetStr(hparm, pszName, GFMNU_ATTR_IMAGE, "");

    if (!bFromTemplate || x == GFUI_TPL_X)
        x = (int)GfParmGetNum(hparm, pszName, GFMNU_ATTR_X, NULL, 0.0);

    if (!bFromTemplate || y == GFUI_TPL_Y)
        y = (int)GfParmGetNum(hparm, pszName, GFMNU_ATTR_Y, NULL, 0.0);

    if (!bFromTemplate || w == GFUI_TPL_WIDTH)
        w = (int)GfParmGetNum(hparm, pszName, GFMNU_ATTR_WIDTH, NULL, 100.0);

    if (!bFromTemplate || h == GFUI_TPL_HEIGHT)
        h = (int)GfParmGetNum(hparm, pszName, GFMNU_ATTR_HEIGHT, NULL, 100.0);
...
static int
createGallery(void* hscr, void* hparm, const char* pszName)
{
    const char *pszDirname = GfParmGetStr(hparm, pszName, GFMNU_ATTR_DIR, "");
    GfuiScreenSetGallery(hscr, pszDirname);
    return 1;
}

int
GfuiMenuCreateStaticImageControl(void* hscr, void* hparm, const char* pszName,
                                 bool bFromTemplate, int x, int y,
                                 int width, int height)
{
    std::string strControlPath =
        bFromTemplate ? GFMNU_SECT_TEMPLATE_CONTROLS"/"
        : GFMNU_SECT_DYNAMIC_CONTROLS"/";
    strControlPath += pszName;

    return createStaticImage(hscr, hparm, strControlPath.c_str(),
        bFromTemplate, x, y, width, height);
}

void
gfuiInitLanguage()
{
    const char *path = "config/language" PARAMEXT;

    if (!(langParam = GfParmReadFileLocal(path, GFPARM_RMODE_STD)))
    {
        GfLogError("GfParmReadFileLocal failed: %s\n", path);
        throw std::runtime_error("Could not open language config");
    }
}
...
{
    if (langParam)
        GfParmReleaseHandle(langParam);

    langParam = nullptr;
}

const char *GfuiGetTranslatedText(void* hparm, const char* pszPath,
    const char* pszKey, const char* pszDefault)
{
    const char *ret = pszDefault, *lang;
    std::string key;
    ...
    if (!langParam)
    {
        ...
        langParam = GfParmReadFileLocal(path, GFPARM_RMODE_STD);
    }

    lang = GfParmGetStr(langParam, "Language", "language", nullptr);
    key = pszKey;
    ...
    if (!(ret = GfParmGetStr(hparm, pszPath, key.c_str(), nullptr)))
        ret = GfParmGetStr(hparm, pszPath, pszKey, pszDefault);
            ...
            const float* aFgFocusColor = GFUI_TPL_FOCUSCOLOR)
{
    ...
    const char* pszText =
        ...
        ? text : GfuiGetTranslatedText(hparm, pszPath, GFMNU_ATTR_TEXT, "");
    const int nX =
    ...
    const char* pszTip = GfuiGetTranslatedText(hparm, pszPath, GFMNU_ATTR_TIP, nullptr);
                 ...
                 const float* aFgPushedColor = GFUI_TPL_PUSHEDCOLOR)
{
    ...
    const char* pszTip =
        ...
        ? tip : GfuiGetTranslatedText(hparm, pszPath, GFMNU_ATTR_TIP, "");
    const int nX =
        bFromTemplate && x != GFUI_TPL_X
        ? x : (int)GfParmGetNum(hparm, pszPath, GFMNU_ATTR_X, NULL, 0);
    const int nY =
    ...
    if (pszTip && strlen(pszTip) > 0)
    {
        tMenuCallbackInfo * cbinfo = (tMenuCallbackInfo*)calloc(1, sizeof(tMenuCallbackInfo));
        cbinfo->screen = hscr;
        cbinfo->labelId = tipLabel = GfuiTipCreate(hscr, pszTip, strlen(pszTip));
        GfuiVisibilitySet(hscr, cbinfo->labelId, GFUI_INVISIBLE);
    ...
    int butId = GfuiButtonCreate(hscr, pszText, nFontId,
                                 ...
                                 userDataOnFocus, onFocus, onFocusLost, tipLabel);
                          ...
                          void* userDataOnFocus, tfuiCallback onFocus, tfuiCallback onFocusLost)
{
    ...
    const char* pszText = GfuiGetTranslatedText(hparm, strControlPath.c_str(), GFMNU_ATTR_TEXT, "");
                              ...
                              void* userData, tfuiComboboxCallback onChange)
{
    ...
    const char* pszTip = GfuiGetTranslatedText(hparm, strControlPath.c_str(), GFMNU_ATTR_TIP, nullptr);
    ...
    id = GfuiComboboxCreate(hscr, font, x, y, width, nArrowsWidth, nArrowsHeight,
                            ...
                            userData, onChange, userDataOnFocus, onFocus, onFocusLost, tipLabel);
...
GfuiMenuCreateCheckboxControl(void* hscr, void* hparm, const char* pszName,void* userData,tfuiCheckboxCallback onChange)
{
    ...
    const char* pszTip = GfuiGetTranslatedText(hparm, strControlPath.c_str(), GFMNU_ATTR_TIP, "");
...
GfuiMenuCreateStaticControls(void* hscr, void* hparm)
{
    ...
    for (int i = 1; i <= GfParmGetEltNb(hparm, GFMNU_SECT_STATIC_CONTROLS); i++)
    {
        ...
        else if (!strcmp(pszType, GFMNU_TYPE_GALLERY))
        {
            createGallery(hscr, hparm, buf);
        }
        else
        {
...
void GfuiMenuScreen::createMenu(float* bgColor,
                                ...
                                int mouseAllowed)
{
    menuHdle = GfuiScreenCreate(bgColor, userDataOnActivate, onActivate,
                                          userDataOnDeactivate, onDeactivate, mouseAllowed);
}

bool GfuiMenuScreen::openXMLDescriptor()
{
    xmlDescParmHdle = GfuiMenuLoad(strXMLDescFileName.c_str());
    return xmlDescParmHdle != 0;
}
...
bool GfuiMenuScreen::closeXMLDescriptor()
{
    if (xmlDescParmHdle)
    {
        GfParmReleaseHandle(xmlDescParmHdle);
        xmlDescParmHdle = 0;
        return true;
    }
    return false;
...
bool GfuiMenuScreen::createStaticControls()
{
    ...
    return menuHdle && xmlDescParmHdle
           && ::GfuiMenuCreateStaticControls(menuHdle, xmlDescParmHdle);
}
                                        ...
                                        void* userDataOnFocus, tfuiCallback onFocus, tfuiCallback onFocusLost)
{
    ...
    if (mapControlIds.find(pszName) == mapControlIds.end())
    {
        const int nCtrlId =
            ::GfuiMenuCreateButtonControl(menuHdle, xmlDescParmHdle, pszName,
                                          userDataOnPush, onPush,
                                          userDataOnFocus, onFocus, onFocusLost);
        if (nCtrlId >= 0)
            mapControlIds[pszName] = nCtrlId;
                                            ...
                                            const float* fgColor, const float* fgFocusColor, const float* fgPushedColor)
{
    ...
    {
        const int nCtrlId =
            ::GfuiMenuCreateTextButtonControl(menuHdle, xmlDescParmHdle, pszName,
                                              userDataOnPush, onPush,
                                              userDataOnFocus, onFocus, onFocusLost,
                                              bFromTemplate,
                                              text, tip, x, y, width, font, textHAlign,
                                             ...
                                             int x, int y, int width, int height)
{
    ...
    {
        const int nCtrlId =
            ::GfuiMenuCreateImageButtonControl(menuHdle, xmlDescParmHdle, pszName,
                                               userDataOnPush, onPush,
                                               userDataOnFocus, onFocus, onFocusLost,
                                               bFromTemplate,
                                               tip, x, y, width, height);
...
int GfuiMenuScreen::createStaticImageControl(const char* pszName)
{
    ...
    {
        const int nCtrlId =
            ::GfuiMenuCreateStaticImageControl(menuHdle, xmlDescParmHdle, pszName);
        if (nCtrlId >= 0)
            mapControlIds[pszName] = nCtrlId;
                                       ...
                                       const float* aFgColor, const float* aFgFocusColor)
{
    ...
    {
        const int nCtrlId =
            ::GfuiMenuCreateLabelControl(menuHdle, xmlDescParmHdle, pszName,
                                      ...
                                      tfuiCallback onFocusLost)
{
    ...
    {
        const int nCtrlId =
            ::GfuiMenuCreateEditControl(menuHdle, xmlDescParmHdle, pszName,
                              userDataOnFocus, onFocus, onFocusLost);
        if (nCtrlId >= 0)
            mapControlIds[pszName] = nCtrlId;
                                            ...
                                            void* userData, tfuiCallback onSelect)
{
    ...
    {
        const int nCtrlId =
            ::GfuiMenuCreateScrollListControl(menuHdle, xmlDescParmHdle, pszName,
                                              userData, onSelect);
        if (nCtrlId >= 0)
            mapControlIds[pszName] = nCtrlId;
                                          ...
                                          void* userData, tfuiComboboxCallback onChange)
{
    ...
    {
        const int nCtrlId =
            ::GfuiMenuCreateComboboxControl(menuHdle, xmlDescParmHdle, pszName,
                                            userData, onChange);
        if (nCtrlId >= 0)
            mapControlIds[pszName] = nCtrlId;
                                          ...
                                          void* userData, tfuiCheckboxCallback onChange)
{
    ...
    {
        const int nCtrlId =
            ::GfuiMenuCreateCheckboxControl(menuHdle, xmlDescParmHdle, pszName,
                                            userData, onChange);
        if (nCtrlId >= 0)
            mapControlIds[pszName] = nCtrlId;
...
int GfuiMenuScreen::createProgressbarControl(const char* pszName)
{
    ...
    {
        const int nCtrlId =
            ::GfuiMenuCreateProgressbarControl(menuHdle, xmlDescParmHdle, pszName);
        if (nCtrlId >= 0)
            mapControlIds[pszName] = nCtrlId;
...
void GfuiMenuScreen::addDefaultShortcuts()
{
    ...
    GfuiMenuDefaultKeysAdd(menuHdle);
}
...
int GfuiMenuScreen::getDynamicControlId(const char* pszName) const
{
    ...
    return iterCtrlId == mapControlIds.end() ? -1 : (*iterCtrlId).second;
}
