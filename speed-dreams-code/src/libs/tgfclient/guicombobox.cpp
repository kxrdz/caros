...
/** Returns whether a combobox is focused.
    @ingroup	gui
    @param	scr		Screen
    @param	id		Button Id
 ...
 */
bool
GfuiComboboxGetFocused(void* scr, int id)
{
	tGfuiObject *object = gfuiGetObject(scr, id);
	if (object && object->widget == GFUI_COMBOBOX)
	{
		const tGfuiCombobox *combo = &object->u.combobox;

		return combo->is_focused;
	}
				   ...
				   int tipLabel)
{
	...
	gfuiGrButtonInit(&combobox->backgroundButton,
					 ...
					 GFUI_MIRROR_NONE,
					 GFUI_MOUSE_UP, NULL, 0, 0, 0, 0, tipLabel);
...
const std::vector<std::string> *GfuiComboboxGetChoices(void* scr, int id)
{
	tGfuiCombobox* combo = gfuiGetCombobox(scr, id);

	if (combo)
		return &combo->pInfo->vecChoices;
