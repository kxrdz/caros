...
TGFCLIENT_API void GfuiGallery(void);
...
TGFCLIENT_API void GfuiScreenSetGallery(void *scr, const char *dirname);
...
public:
    ...
    void setNextMenuHandle(void* hdle)
    {
        nextMenuHdle = hdle;
    }
    ...
    void* getXMLDescriptor() const
    {
        return xmlDescParmHdle;
    }
...
#define GFUI_FONT_NB	       12 // Warning: Keep always this sync'd with GFUI_FONT_* above !
...
TGFCLIENT_API std::string GfuiLabelGetText(void* scr, int id);
...
TGFCLIENT_API int GfuiGrButtonCreate(void* scr, const char* disabled, const char* enabled,
                                     ...
                                     void* userDataOnFocus, tfuiCallback onFocus, tfuiCallback onFocusLost,
                                     int tipLabel = -1);
TGFCLIENT_API bool GfuiGrButtonGetFocused(void* scr, int id);
...
TGFCLIENT_API const std::vector<std::string> *GfuiComboboxGetChoices(void* scr, int id);
TGFCLIENT_API unsigned GfuiComboboxGetNumberOfChoices(void* scr, int id);
TGFCLIENT_API bool GfuiComboboxGetFocused(void* scr, int id);
...
TGFCLIENT_API bool GfuiButtonGetFocused(void* scr, int id);
...
TGFCLIENT_API int GfuiScrollListAppendElement(void* scr, int Id, const char* element, void* userData);
...
TGFCLIENT_API int GfuiMenuCreateEditControl(void* hscr, void* hparm, const char* pszName,
                                            void* userDataOnFocus = nullptr,
                                            tfuiCallback onFocus = nullptr,
                                            tfuiCallback onFocusLost = nullptr);
...
TGFCLIENT_API int GfuiMenuCreateComboboxControl(void* hscr, void* hparm, const char* pszName,
                                                ...
                                                tfuiComboboxCallback onChange = nullptr);
...
TGFCLIENT_API int GfTexTryPNGImage(const char *filename);
TGFCLIENT_API int GfTexTryJPEGImage(const char *filename);
...
class TGFCLIENT_API GfuiEventLoop : public GfEventLoop
{
  public: // Member functions.
    ...
    //! Set the "predisplay" callback function.
    void setPredisplayCB(void (*func)(void));

    //! Set the "redisplay/refresh" callback function.
    void setRedisplayCB(void (*func)(void));

    //! Call any actions required before rendering the screen.
    void predisplay();
