...
static void
gfuiScrollListUpdateScroll(void *scr, tGfuiScrollList *scrollist, int showElt = -1, int elemBeforeAfter = 0)
{
...
gfuiScrollListNextElt (tGfuiObject *object)
{
	...
	gfuiScrollListUpdateScroll(GfuiScreen, scrollist, scrollist->selectedElt);
}
...
GfuiScrollListSetSelectedElement(void *scr, int id, unsigned int selectElement)
{
	...
	gfuiScrollListUpdateScroll(scr, scrollist, scrollist->selectedElt);
    return true;
}
...
GfuiScrollListExtractElement(void *scr, int id, int index, void **userData)
{
    ...
    *userData = elt->userData;
	...
	gfuiScrollListUpdateScroll(scr, scrollist);
    return name;
}
...
GfuiScrollListInsertElement(void *scr, int id, const char *element, int index, void *userData)
{
    ...
    char *s = (char *)malloc(strlen(element) + 1);
    strcpy(s, element);
    elt->name = s;
    elt->label = s;
    ...
    scrollist->nbElts++;
	gfuiScrollListUpdateScroll(scr, scrollist, scrollist->nbElts - 1);

    return 0;
}

/** Insert an element to the end of a scroll list.
    @ingroup	gui
    @param	scr		Current screen
    @param	id		Scroll list Id
    @param	element		New element
...
*/
int
GfuiScrollListAppendElement(void* scr, int Id, const char* element, void* userData)
{
  int pos = GfuiScrollListGetNumberOfElements(scr, Id);

  if (pos < 0)
    return -1;

  return GfuiScrollListInsertElement(scr, Id, element, pos, userData);
}
...
GfuiScrollListShowElement(void *scr, int id, int index)
{
	...
	gfuiScrollListUpdateScroll(scr, scrollist, index);
}
...
GfuiScrollListMoveSelectedElement(void *scr, int id, int delta)
{
	...
	gfuiScrollListUpdateScroll(scr, scrollist, scrollist->selectedElt, abs(delta));
